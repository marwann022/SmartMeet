<template>
  <div class="card-glass rounded-[28px] overflow-hidden border border-white/80 dark:border-white/5 shadow-glass backdrop-blur-md">
    <div class="overflow-x-auto">
      <table class="table w-full text-left border-collapse">
        <thead>
          <tr class="bg-black/[0.02] dark:bg-white/[0.02] border-b border-black/5 dark:border-white/5 text-[11px] font-extrabold text-brand-slate tracking-wider uppercase">
        ...
        <tbody class="divide-y divide-black/5 dark:divide-white/5 text-sm">
          <tr v-if="loading">
            <td colspan="5" class="py-8">
              <div class="flex flex-col gap-4">
                <div v-for="i in 5" :key="i" class="h-12 bg-black/5 dark:bg-white/5 animate-pulse rounded-xl w-full"></div>
              </div>
            </td>
          </tr>
