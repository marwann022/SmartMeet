    ...
    <div class="flex flex-col items-center justify-center gap-12 py-10 px-4">
      <div class="w-full max-w-[480px] bg-white/70 border border-white/70 backdrop-blur-lg rounded-[32px] p-8 sm:p-12 shadow-glass flex flex-col gap-7 items-center transition-all duration-300 hover:border-white/95 hover:shadow-card-hover group/card">
        <template v-if="!registrationSuccess">
          ...
          <!-- SSO Authentication -->
          <div class="w-full">
            <button type="button" class="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-black/8 bg-white/50 text-[11px] font-bold text-brand-dark transition-all duration-300 hover:bg-white hover:border-primary/20 hover:shadow-sm cursor-pointer" @click="handleSSO('Google')">
        ...
        <!-- Beautiful Registration Success State -->
        <div v-else class="flex flex-col items-center justify-center text-center py-6 w-full animate-fade-in">
          ...
          <router-link
            to="/signin"
            class="w-full py-3.5 px-6 rounded-xl bg-primary hover:bg-[#3850e0] text-white text-xs font-bold tracking-wider uppercase hover:shadow-md transition-all duration-300 cursor-pointer text-center"
          >
            Proceed to Log In
          </router-link>
        </div>
...
import { PhEnvelope, PhLock, PhUser, PhShieldCheck, PhWarningCircle, PhCheckCircle } from '@phosphor-icons/vue'
